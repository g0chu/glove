...
- **Turn** — `runToolTurn` (`tools/loop.ts`): model ↔ tool rounds capped by `TOOLS_MAX_ROUNDS`; tool results live only in the turn. Then `ResponseWriter` posts the final reply. Only the final reply reaches channel history; tool-activity (🔎/📁/🐚/🔧) lines, thinking (🤔) lines, and the intermediate rounds' text (settled in place between the activity lines) are posted but never tracked.
...
- Plain `node:assert/strict`; `ok(name)` check groups (currently **118**); the final line prints the count.
