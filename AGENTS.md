...
- `src/` — the bot (compiled to `dist/`):
  ...
  - `config.ts` (env parsing), `log.ts` (logging/error helpers), `bot/` (client, router, queue, writer, format), `llm/` (client, history), `tools/` (registry/executor, tool loop, in-process `WebTools` + `FileTools` and their `web/`/`file/` modules).
...
- `TODO.md` — open work (currently: live-verification of the in-process tool turns); `README.md` — user-facing docs incl. the tools section.
- `.venv/` — obsolete Python venv left on disk by the removed sidecar; gitignored, safe to delete. Ignore it; the project is TypeScript-only.
...
- **Writer** (`bot/writer.ts`): Discord's 2000-char limit; `splitForDiscord` never splits inside a code fence (closes it, reopens in the next chunk), keeps markdown tables together (a table that must span messages repeats its header + separator row in every part; a title line right before the table is carried into the table's chunk), and prefers paragraph/heading/list boundaries when breaking prose; live-stream preview edits are throttled by `throttleMs`; `discard()` drops an in-progress preview without finishing the turn (used between tool rounds). Model text is sanitized by `bot/format.ts` before posting (and before it lands in history): a `DISCORD_FORMAT_NOTE` is appended to every system prompt (no LaTeX — Discord renders markdown only — use Unicode symbols, keep tables within one message), and any `$...$` math the model still emits is rewritten to plain Unicode (`$\text{VO}_2 \text{max}$` → `VO₂ max`, `$\uparrow$` → `↑`); non-math `$` (prices) is left alone.
...
- `npm test` — bot smoke tests: one script (`test/smoke.ts`), plain `node:assert/strict`, no framework, no test selection (57 check groups). Hermetic: mock OpenAI-compatible server on an ephemeral port, fake Discord channels, injected DNS/search backends, temp workspace directories; no `.env`, Discord, Docker, or network access. Covers config parsing, history window, code-fence/table chunking, LaTeX sanitization, queue, writer (incl. `discard()` and sanitization), the executor, the tool loop, the in-process web tools (SSRF guard, cache, extraction, search) and file tools (path confinement, all file ops), and the LLM client (SSE, non-stream, errors, timeouts, `abort()`, tool-call wire shapes).
...
- Naming: PascalCase types/classes, camelCase functions/methods, `UPPER_SNAKE_CASE` constants (`DISCORD_MAX_MESSAGE_CHARS`, `BLOCKED_V4`), numeric separators in large literals (`200_000`).
